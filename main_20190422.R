...
dataset = dataset[dataset$START_DATETIME<'2017-01-01',]
saveRDS(dataset,'dataset.rds')

dataset = as.data.frame(dataset)
dim(dataset)  #NEW 690874*320
str(dataset)
...
# 無意義欄位
droplist4 = c('ENGINE_NO','INSURE','CATEGORY_CODE','POLICY_RESULT_CODE','COUNSEL_DIV_NO','AGENT_DIV_NO')
...
dim(train_data) #513639*277
dim(test_data) #177235*277
...
test_id = test_data$CONTRACT_NO
train_data$CONTRACT_NO=NULL
train_data$GROUP=NULL
test_data$CONTRACT_NO=NULL
test_data$GROUP=NULL

train_x = train_data[,-which(colnames(train_data)=='LOSS_RATE')]
...
for (this.round in 1:nrounds){      
  ...
  saveRDS(model, file = paste0("./model/model_xgb_",this.round,".rda"))
...
pred_y = ifelse(pred_test>0.095,'1','0') 
...
df = data.frame(CONTRACT=test_id,PRED=pred_y)
